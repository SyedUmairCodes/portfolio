...
export default function ProjectsSection() {
  return (
    <section
      className="min-h-screen py-24 px-6 sm:px-12 flex justify-center text-white bg-background"
      id="projects"
    >
...
function ProjectCard({ project }: { project: (typeof PROJECTS)[0] }) {
  return (
    <motion.div
      ...
      className="group relative h-[400px] md:h-[500px] w-full rounded-2xl overflow-hidden"
    >
      {/* Background Image */}
      ...
      <div className="absolute inset-0 p-8 md:p-12 flex flex-col justify-end">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
          ...
          <div className="md:col-span-7 flex flex-col gap-2">
            <span className="text-lg md:text-xl mb-2 text-white">
              / {project.id}
            </span>
            ...
            <p className="text-sm font-bold tracking-wider uppercase text-white mt-1">
              {project.category}
            </p>
          </div>
          <div className="md:col-span-5 md:pl-8">
            <p className="text-base md:text-lg text-white line-clamp-3  transition-colors duration-300">
              {project.description}
            </p>

            {/* Hover Indicator Line */}
            <div className="h-0.5 w-0 group-hover:w-full mt-6 transition-all duration-700 ease-in-out bg-white" />
          </div>
        </div>
      </div>
